...
with st.sidebar:
    ...
    user_device_overall = pd.read_sql(query6, con=connection)
    ...
    fig = px.pie(df, values="Total Transactions count", names='Payment Mode', title='Total Transactions count')
    fig.update_layout(height=350, width=350)
    st.plotly_chart(fig)

    #import plotly.graph_objects as go
    #labels = user_device["Brand Name"]
    #values =  user_device["Registered Users Count"]
    #fig1 = go.Figure(data=[go.Pie(labels=labels, values=values,hole=0.3,title="Registered user by brand name")])
